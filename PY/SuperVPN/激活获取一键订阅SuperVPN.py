...
def post_to_dpaste(content):
    ...
    response = requests.post(dpaste_url, data=data)
    if response.status_code == 201:
        # 获取响应链接并添加 .txt 后缀
        return response.headers['Location'] + '.txt'
    else:
        print("Failed to post to dpaste")
        return None
...
def n():
    ...
    # 将链接发布到 dpaste
    content = '\n'.join(Hyy)
    dpaste_link = post_to_dpaste(content)
    if dpaste_link:
        print(f"订阅: {dpaste_link}")
